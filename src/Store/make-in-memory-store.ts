...
import { LabelAssociation, LabelAssociationType } from '../Types/LabelAssociation';
...
export default (config: BaileysInMemoryStoreConfig) => {
  ...
  const bind = (ev: BaileysEventEmitter) => {
    ...
    ev.on('messages.update', (updates: { update: Partial<WAMessage>; key: WAMessageKey }[]) => {
    ...
    ev.on('messages.delete', (item: { all: true; jid: string } | { keys: WAMessageKey[] }) => {
      if ('all' in item) {
    ...
    ev.on('message-receipt.update', (updates: { key: WAMessageKey; receipt: any }[]) => {
    ...
    ev.on('messages.reaction', (reactions: { key: WAMessageKey; reaction: any }[]) => {
  ...
  return {
    ...
    loadMessages: async (jid: string, count: number, cursor: WAMessageCursor): Promise<proto.IWebMessageInfo[]> => {
      const list = assertMessageList(jid);
    ...
    loadMessage: async (jid: string, id: string): Promise<proto.IWebMessageInfo | undefined> => {
      return messages[jid]?.get(id);
    },

    mostRecentMessage: async (jid: string): Promise<proto.IWebMessageInfo> => {
      const message = messages[jid]?.array.slice(-1)[0];
      return message!;
    },

    fetchImageUrl: async (jid: string, sock: WASocket | undefined): Promise<string | null | undefined> => {
    ...
    fetchGroupMetadata: async (jid: string, sock: WASocket | undefined): Promise<GroupMetadata> => {
    ...
    fetchMessageReceipts: async ({ remoteJid, id }: WAMessageKey): Promise<proto.IUserReceipt[] | null | undefined> => {
