    ...
    for (let i = 0; i < samples; i++) {
      ...
      for (let j = 0; j < blockSize; j++) {
        ...
        if (index < rawData.length) {
          const sample = rawData[index];
          // Pastikan sample adalah number yang valid
      ...
      // Hitung rata-rata hanya dengan sample yang valid
      const average = validSampleCount > 0 ? sum / validSampleCount : 0;
      filteredData.push(average);
    }

    // Handle kasus dimana semua data adalah 0
...
export const getUrlFromDirectPath = (directPath: string) =>
