...
export const makeCommunitiesSocket = (config: SocketConfig) => {
  ...
  return {
      ...
      const result = await communityQuery(jid, "set", [
        {
          ...
          attrs: action === "reject" ? { linked_groups: "true" } : {},
          content: [
            {
              tag: action,
              attrs: {},
    ...
    communityParticipantsUpdate: async (
      jid: string,
      participants: string[],
    ...
    communityToggleEphemeral: async (
      jid: string,
...
export const extractCommunityMetadata = (result: BinaryNode) => {
  ...
  const metadata: GroupMetadata = {
    ...
    addressingMode: getBinaryNodeChildString(
      community,
      "addressing_mode",
    )! as GroupMetadata["addressingMode"],
  };
  return metadata;
};
