...
const makeSocket = (config) => {
    ...
    const onWhatsApp = async (...jids) => {
        let usyncQuery = new WAUSync_1.USyncQuery().withLIDProtocol();
        ...
        if (results) {
            ...
            if (lidOnly.length > 0) {
                const pairs = lidOnly.map(a => ({ pn: a.id, lid: a.lid }));
    ...
    const validateConnection = async () => {
        ...
        const handshake = (0, proto_utils_1.decodeAndHydrate)(index_js_1.proto.HandshakeMessage, result);
