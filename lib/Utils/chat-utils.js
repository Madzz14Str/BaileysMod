...
const decodeSyncdMutations = async (msgMutations, initialState, getAppStateSyncKey, onMutation, validateMacs) => {
    ...
    for (const msgMutation of msgMutations) {
        ...
        const syncAction = (0, proto_utils_1.decodeAndHydrate)(index_js_1.proto.SyncActionData, result);
        if (validateMacs) {
...
const extractSyncdPatches = async (result, options) => {
    ...
    await Promise.all(collectionNodes.map(async (collectionNode) => {
        ...
        if (snapshotNode && !!snapshotNode.content) {
            ...
            const blobRef = (0, proto_utils_1.decodeAndHydrate)(index_js_1.proto.ExternalBlobReference, snapshotNode.content);
            const data = await (0, exports.downloadExternalBlob)(blobRef, options);
            snapshot = (0, proto_utils_1.decodeAndHydrate)(index_js_1.proto.SyncdSnapshot, data);
        }
        for (let { content } of patches) {
            if (content) {
                ...
                const syncd = (0, proto_utils_1.decodeAndHydrate)(index_js_1.proto.SyncdPatch, content);
                if (!syncd.version) {
...
const downloadExternalPatch = async (blob, options) => {
    ...
    const syncData = (0, proto_utils_1.decodeAndHydrate)(index_js_1.proto.SyncdMutations, buffer);
    return syncData;
};
